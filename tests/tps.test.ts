// Tests for the token-throughput tracker in `tps.ts`.
//
// The engine feeds the composer row through `footer.tsx`; these are its unit
// tests, and `footer.test.ts` covers the wiring (event subscriptions, the
// refresh timer) plus the rendered rate, including the frozen average.
...
describe("TpsTracker", () => {
  test("the shipped live-estimate ratio is the documented default", () => {
    // The estimated-token pins in this suite are ceil(bytes / bytesPerToken)
    // at this ratio, so changing it should fail here first.
    expect(DEFAULT_CONFIG.bytesPerToken).toBe(4.75)
  })
  ...
  test("honours a custom live bytes-per-token ratio", () => {
    const tracker = new TpsTracker({ bytesPerToken: 5 })
  ...
  test("caps how many finished runs it remembers, keeping the recent ones", () => {
    ...
    // One run past the cap, one run per session: the first started is the first forgotten.
    for (let i = 0; i < MAX_TRACKED_RUNS + 1; i += 1) {
    ...
    expect(tracker.value(`s${MAX_TRACKED_RUNS}`, 10_000)?.frozen).toBe(true)
  })
  ...
  test("re-applies the session cap when an oversized running set finishes", () => {
    ...
    for (let i = 0; i < MAX_TRACKED_RUNS + 1; i += 1) tracker.push(`s${i}`, DELTA, i)
  ...
  test("missing streamed event falls back to the content boundary without charging tool wait", () => {
    ...
    tracker.finishStep("s", "m1", 20, 10_000)
    // Live at the content boundary: 20 tokens over 200 ms would exceed the
    // minimum denominator, so the 250 ms floor applies: 80 t/s.
...
describe("formatTpsLabel", () => {
  const value: TpsValue = {
    tps: 6.5,
    tokens: 41,
    frozen: false,
  ...
  test("prints the rate and omits the token count", () => {
    expect(formatTpsLabel(value)).toBe("~6.50 t/s")
  })

  test("scales precision with magnitude", () => {
    expect(formatTpsLabel({ ...value, tps: 62.44 })).toBe("~62.4 t/s")
    expect(formatTpsLabel({ ...value, tps: 184.6 })).toBe("~185 t/s")
  })

  test("uses a dash when no rate is available", () => {
    expect(formatTpsLabel({ ...value, tps: null })).toBe("— t/s")
  })
})

describe("resolveTpsOptions", () => {
  test("empty input yields the defaults", () => {
    expect(resolveTpsOptions({})).toEqual(DEFAULT_TPS_OPTIONS)
  })

  test("accepts valid values", () => {
    const options = resolveTpsOptions({ refreshHz: 20, bytesPerToken: 5 })
  ...
  test("clamps numbers into their supported range", () => {
    expect(resolveTpsOptions({ refreshHz: REFRESH_HZ_MIN - 1 }).refreshHz).toBe(REFRESH_HZ_MIN)
    expect(resolveTpsOptions({ refreshHz: REFRESH_HZ_MAX + 1 }).refreshHz).toBe(REFRESH_HZ_MAX)
    expect(resolveTpsOptions({ bytesPerToken: BYTES_PER_TOKEN_MIN - 1 }).bytesPerToken).toBe(BYTES_PER_TOKEN_MIN)
    expect(resolveTpsOptions({ bytesPerToken: BYTES_PER_TOKEN_MAX + 1 }).bytesPerToken).toBe(BYTES_PER_TOKEN_MAX)
  })

  test("rejects invalid values", () => {
    const options = resolveTpsOptions({ refreshHz: "12", bytesPerToken: null })

    expect(options.refreshHz).toBe(DEFAULT_TPS_OPTIONS.refreshHz)
    expect(options.bytesPerToken).toBe(DEFAULT_TPS_OPTIONS.bytesPerToken)
  })

  test("tolerates hostile shapes", () => {
    expect(resolveTpsOptions({ refreshHz: Number.NaN, bytesPerToken: Number.POSITIVE_INFINITY })).toEqual(
      DEFAULT_TPS_OPTIONS,
    )
  })
})
